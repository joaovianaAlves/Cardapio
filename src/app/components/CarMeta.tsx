import React from "react";
import Image from "next/image";

const cardInfo = [
  {
    image: "/meta/Meta.jfif",
    title: "Workrooms",
    description:
      "Uma plataforma de colaboração virtual projetada para reuniões e trabalho em equipe, oferecendo salas de reunião personalizadas e ferramentas de produtividade.",
  },
  {
    image: "/meta/FirstTouch.jfif",
    title: "First Touch",
    description:
      "Uma experiência introdutória que permite aos usuários explorar e interagir com o ambiente virtual do Oculus, oferecendo uma introdução tátil ao mundo da realidade virtual.",
  },
  {
    image: "/meta/BeatSaber.jfif",
    title: "Beat Saber Demo",
    description:
      "Um jogo de ritmo frenético que combina música, luzes e ação, onde os jogadores cortam blocos com sabres de luz ao ritmo da música.",
  },
  {
    image: "/meta/HandsPhysics.jpg",
    title: "Hands Physics",
    description:
      "Uma aplicação que demonstra a física realista das mãos e dos objetos em um ambiente virtual, proporcionando uma experiência imersiva de interação.",
  },
  {
    image: "/meta/FirstHand.jpg",
    title: "First Hand",
    description:
      "Uma experiência imersiva que coloca o usuário em diferentes situações do dia a dia, permitindo uma abordagem prática e interativa.",
  },
  {
    image: "/meta/FirstContact.jpg",
    title: "First Contact",
    description:
      "Uma experiência introdutória que apresenta aos usuários os fundamentos da interação no Oculus VR, incluindo manipulação de objetos e interação com personagens virtuais.",
  },
  {
    image: "/meta/FirstSteps.jfif",
    title: "First Steps",
    description:
      "Uma introdução interativa ao Oculus VR, que orienta os usuários através de diversas atividades e demonstra as capacidades do dispositivo.",
  },
  {
    image: "/meta/Flipside.jfif",
    title: "Flipside",
    description:
      "Uma plataforma de criação e transmissão de conteúdo em realidade virtual, que permite aos usuários produzirem e compartilharem seus próprios programas e experiências.",
  },
  {
    image: "/meta/Immerse.jfif",
    title: "Immerse",
    description:
      "Uma aplicação que oferece experiências imersivas em 360 graus, permitindo aos usuários explorarem locais reais e virtuais em todo o mundo.",
  },
  {
    image: "/meta/Golf.jfif",
    title: "Golf+",
    description:
      "Uma simulação de golfe em realidade virtual, que oferece uma experiência realista de jogo em campos virtuais deslumbrantes.",
  },
  {
    image: "/meta/BigScreen.jfif",
    title: "BigScreen",
    description:
      "Uma plataforma de compartilhamento de tela em realidade virtual, que permite aos usuários assistir a filmes, jogar jogos e trabalhar juntos em salas virtuais.",
  },
  {
    image: "/meta/3DOrganon.jfif",
    title: "3D Organon",
    description:
      "Aplicativo educativo de anatomia em realidade virtual. Oferece modelos tridimensionais detalhados do corpo humano, permitindo aos usuários explorar sistemas e órgãos.",
  },
  {
    image: "/meta/JurassicWord.jfif",
    title: "Jurassic World",
    description:
      "Uma experiência imersiva que transporta os usuários para o mundo dos dinossauros, permitindo explorar e interagir com essas criaturas pré-históricas em seu habitat natural.",
  },
  {
    image: "/meta/Prisms.png",
    title: "Prisme Math",
    description:
      "Uma aplicação educativa que utiliza realidade virtual para ensinar conceitos matemáticos de uma forma interativa e envolvente.",
  },
  {
    image: "/meta/MissionISS.jfif",
    title: "Mission ISS",
    description:
      "Uma experiência que simula a vida a bordo da Estação Espacial Internacional, permitindo aos usuários explorarem e realizarem tarefas como astronautas.",
  },
  {
    image: "/meta/GymClass.jfif",
    title: "Gym Class",
    description:
      "Uma aplicação de fitness em realidade virtual, que oferece basquete em um ambiente imersivo.",
  },
  {
    image: "/meta/Alcove.jfif",
    title: "Alcove",
    description:
      "Uma experiência interativa que permite aos usuários explorarem memórias e momentos especiais em um ambiente virtual relaxante.",
  },
  {
    image: "/meta/Arkio.jfif",
    title: "Arkio",
    description:
      "Uma aplicação de design e colaboração em arquitetura, que permite aos usuários criarem e explorarem modelos tridimensionais de edifícios e espaços.",
  },
  {
    image: "/meta/GravitySketch.jfif",
    title: "Gravity Sketch",
    description:
      "Uma ferramenta de design em realidade virtual que permite aos usuários criarem modelos tridimensionais de forma intuitiva e imersiva.",
  },
  {
    image: "/meta/Nanome.jfif",
    title: "Nanome",
    description:
      "Uma plataforma de modelagem molecular em realidade virtual, que permite aos cientistas e pesquisadores visualizarem e manipularem estruturas moleculares complexas.",
  },
  {
    image: "/meta/XRWorkout.jfif",
    title: "XR Workout",
    description:
      "Uma aplicação de treino em realidade virtual, que oferece uma variedade de exercícios e rotinas de fitness em ambientes virtuais envolventes.",
  },
  {
    image: "/meta/SpaceExplorer.jfif",
    title: "Space Explorer",
    description:
      "Uma experiência educativa que permite aos usuários explorarem o cosmos e aprenderem sobre astronomia em um ambiente virtual.",
  },
  {
    image: "/meta/NeverBoard.jfif",
    title: "NeverBoard",
    description:
      "Uma aplicação que oferece uma variedade de jogos de tabuleiro em realidade virtual, permitindo aos jogadores desfrutarem de jogos clássicos e novos em um ambiente virtual.",
  },
  {
    image: "/meta/Mlk.jfif",
    title: "MLK",
    description:
      "Uma experiência educativa que homenageia o legado de Martin Luther King Jr., permitindo aos usuários explorarem momentos importantes da sua vida e do movimento dos direitos civis.",
  },
  {
    image: "/meta/Tripp.jfif",
    title: "Tripp",
    description:
      "Uma aplicação de bem-estar em realidade virtual, que oferece experiências de meditação, relaxamento e crescimento pessoal em ambientes virtuais.",
  },
  {
    image: "/meta/SportsScramble.jfif",
    title: "Sports Scramble",
    description:
      "Uma experiência educativa que permite aos usuários explorarem o cosmos e aprenderem sobre astronomia em um ambiente virtual.",
  },
];

const cardItem = () => {
  return cardInfo.map((item, index) => {
    return (
      <div
        key={index}
        className="flex flex-col p-4 items-center border border-gray-300 rounded-2xl shadow-lg w-[90%] mx-auto my-4 bg-white hover:shadow-2xl transition duration-300 ease-in-out cursor-pointer"
      >
        <div>
          <Image
            className="rounded-2xl"
            src={item.image}
            alt=""
            width={200}
            height={150}
          />
        </div>
        <div className="flex flex-col px-5 pt-3">
          <h1 className="text-start font-bold text-lg">{item.title}</h1>
          <p className="text-start text-sm justify">{item.description}</p>
        </div>
      </div>
    );
  });
};

export default function CardMeta() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-5 w-[80%] mx-auto">
      {cardItem()}
    </div>
  );
}
